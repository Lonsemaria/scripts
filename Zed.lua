...
function findorbwalker()
 if _G.Reborn_Loaded then
    SAC=true and
    print("<font color=\"#00BAE4\"><b> LonseViktor - </b></font><font color=\"#CBF6FF\">SACR is Loaded")
...
function Targetselection()
  ...
  tse= TargetSelector(TARGET_LESS_CAST_PRIORITY, 1025, DAMAGE_MAGIC, false, true)
  tsr= TargetSelector(TARGET_LESS_CAST_PRIORITY, 700, DAMAGE_MAGIC, false, true) 
   ...
   ESpell = _Spell({Slot = _E, DamageName = "E", Range = 1025, Width = 90, Delay = 0.25, Speed = 1200, Collision = true, Aoe = false, Type = SPELL_TYPE.LINEAR})
...
function Menu()
      ...
      Config.killsteal:addParam("useW", "Steal With E", SCRIPT_PARAM_ONOFF, true)
...
for _, unit in pairs(GetEnemyHeroes()) do
    local health = unit.health
    local dmgW = getDmg("E", unit, myHero) + ((myHero.ap)*0.7) + ((getDmg("E", unit, myHero) + ((myHero.ap)*0.7))*0.4)
